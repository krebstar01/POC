...
public class TestWatermarkStatusApplication {
	...
	public static final ResourceTestRule resources = ResourceTestRule.builder()
			.addResource(watermarkDao).addResource(new WatermarkStatusApplication(watermarkManager)).build();
    ...
    private void setUpData() {
    	...
    	//when(watermarkManager.processDocument(any(BaseDocument.class))).thenReturn(baseDocument2);
    ...
    public void testRetrieveDocumentByTicket() {
        ...
        WebTarget target = client.target("/status/retrieveDocumentByTicket/111");
    ...
    public void testGetAllBaseDocuments() {
        Client client = resources.client();
        WebTarget target = client.target("/status/retrieveAllDocuments");
        Integer actual = target.request(MediaType.APPLICATION_JSON).get().getStatus();
        assertThat(actual).isEqualTo(200);
    }
    ...
    public void testRetrieveAllWatermarks() {
        Client client = resources.client();
        WebTarget target = client.target("/status/retrieveAllWatermarks");
        Integer actual = target.request(MediaType.APPLICATION_JSON).get().getStatus();
        assertThat(actual).isEqualTo(200);
    }
    ...
    public void testRequestTicketJournal() {
        Client client = resources.client();
        WebTarget target = client.target("/status/requestTicket");
