package com.sample.service.controller.handler;

import com.sample.configuration.GlobalProperties;
...
public class GlobalExceptionHandler {
    ...
    /**
     * a dummy handler handler for the example HelloController, for the endpoint "unknownException"
     ...
     * a com.sample.domain.model.exception.NotFoundException must be explicity thrown
     * if the configuration value "debug"  is set to true, extra information can be passed along to both the client error message as well as the log file
     */
...
/*    *//**
    ...
    void handleBadArgumentException(EOFException e, HttpServletResponse response) throws IOException {

        String errorMessage = "EOFException " + e.getMessage();

        String rootCause = ExceptionUtils.getRootCauseMessage(e);

        if (StringUtils.containsIgnoreCase(rootCause, "Broken pipe")) {

            response.sendError(HttpStatus.GATEWAY_TIMEOUT.value(), errorMessage);
            logger.info("socket is closed, cannot return any response ");

        } else {
            //return new HttpEntity<>(e.getMessage());
            response.sendError(HttpStatus.REQUEST_TIMEOUT.value(), errorMessage);
